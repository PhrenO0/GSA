...
Inherited from [Viscose](https://github.com/Yousuf-developer/Viscose-carousel)
(MIT, Yousuf Soomro) and kept because it is still accurate about the ring.
What this fork changed is in the sections marked **fork**.

## What this is

@kim.gemni's portfolio. The landing page is a carousel; Eighteen project cards sit on a ring that is
...
Everything visible on the landing page is either that one shader pass, a handful
of absolutely positioned DOM labels over the top of it, or the nav. The other
routes (`/works`, `/works/[slug]`, `/about`) are ordinary scrolling documents
with no canvas at all.

**김젬니 is a fictional persona and every figure in `content/` is invented.**
Do not present any of it as a real result, and do not quietly drop the notices
that say so — the footer on every standard page, the block in README.md, and
the paragraph in LICENSE.
...
```
app/
  page.js              <SiteNav> + <Carousel>, nothing else
  layout.js            root layout + metadata, lang="ko"
  globals.css          Tailwind v4 import, @font-face, .ring-locked
  icon.svg             favicon
  works/page.js        the eighteen as a list — the keyboard's way in
  works/[slug]/page.js one work: brief, approach, metrics, outcome
  about/page.js        persona, timeline, awards, FAQ, contact

content/                 fork — the single source of truth
  works.js             the eighteen works, in ring order, with their copy
  profile.js           persona, stats, tracks, timeline, services, FAQ

tools/                   fork — neither runs in `npm run build`
  make-cards.mjs       draws public/*.webp from content/works.js
  subset-fonts.py      Pretendard -> KS X 1001, four weights under 700 KB

components/
  Carousel.jsx        the component. renderer, resize/fit, input, spin
                       physics, the per-frame layout loop, the entry timeline
  SiteNav.jsx         the one bit of persistent chrome
  SiteFooter.jsx      standard pages only, carries the sample-data notice
  ring/
    projects.js        the ring's deal, derived from content/works.js
...
**`PROJECTS` order is ring order.** `ring/projects.js` is a projection of
`content/works.js`, so that array is where the sequence lives: reordering rows
there moves the ring, the column, the numbering and the prev/next links on the
detail pages together. Do not use `imageOffset` for this; it rotates the art
without moving the list. Renaming or re-tracking a work also changes its card,
so re-run `node tools/make-cards.mjs` after editing it.

**Clicking the front card navigates (fork).** `pick(i)` used to return early
when the card was already at the front; that early return now calls
`openRef.current(shown)`. `shown` is the cell the front plane is *wearing*,
read off the same deal the shader was handed — deriving it from the angle
instead would let the highlight and the destination disagree. The ref is how
the render's router reaches an effect that runs once and closes over nothing.
...
1. **Satoshi and Geist are still `.otf`/`.ttf`, ~340 KB.** Converting them to
   `woff2` would cut that by roughly 60%; Pretendard is already woff2.
2. **The card art is drawn at 768x512 and the atlas cell is 512.** Half the
   pixels are thrown away on load. It is 224 KB across eighteen files, so this
   is cheap to leave alone.
3. **`prefers-reduced-motion` is unhandled.** Six seconds of animated blur with
   no escape hatch.
4. **No keyboard control of the ring.** Arrow keys should step it; the project
   column is `pointer-events-none` and cannot be clicked to jump. `/works` is
   the standing workaround, not a fix.
5. **The nav sits over the ring on a phone.** At 390px the wordmark overlaps
   whatever card is passing top-left. It is legible, but it is not placed.
6. **All the content is invented.** `content/` describes a persona who does
   not exist; the card art is generated from it. Flagged in the footer, the
   README and the LICENSE. Do not present it as real work or strip those
   notices.
...
Three families, and **all three are looked up by name**: the strings in
`ring/params.js` (`nameFont`, `idxFont`, `textFont`) have to match a
`@font-face` family in `app/globals.css`, and the `textFont` dropdown in
`gui.js` lists them a third time. A name with no matching block falls back to
system sans silently, which looks like a rendering bug rather than a missing
file.

- **Pretendard** carries the Hangul. `public/fonts/*.subset.woff2` are subsets
  of the KS X 1001 2,350 syllables; a syllable outside that set falls back.
  Widen the set in `tools/subset-fonts.py` rather than committing the full
  780 KB weights.
- **Satoshi** and **Geist** are Latin only. Anything rasterised to canvas — the
  intro heading and the cursor tag — is set in Satoshi *because* it is Latin:
  a Korean string there would need the webfont loaded before `measureText`
  runs, and there is nothing waiting on that. Keep those two strings Latin.
- Geist is used for figures, several of which carry a Hangul unit ("212만"), so
  those stacks name Pretendard after it. Do not trim the fallback.

`word-break: keep-all` is set on `body` and is load-bearing: Korean has no
inter-word breaking to fall back on, and without it lines break mid-word.
